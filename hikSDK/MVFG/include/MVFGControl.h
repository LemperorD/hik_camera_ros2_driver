 ...
 *  @remarks The image size of the stream channel needs to be reacquired after the image-related parameters of the camera are changed, such as width, height, and pixel format.
 ************************************************************************/
MV_FGCTRL_API int __stdcall MV_FG_GetPayloadSizeEx(IN STREAMHANDLE hStream, OUT uint64_t* pnPayloadSize);
 ...
 *  @~english
 *  @brief  Register buffer to the stream channel. 
 *  @param  hStream                 [IN]            Stream channel handle
 ...
 *  @param  phBuffer                [OUT]          缓存句柄
 *  @return 成功，返回MV_FG_SUCCESS；失败返回错误码
 *  @remarks 图像缓存大小通过MV_FG_GetPayloadSizeEx接口获取；私有信息由用户自定义。
 ...
 *  @param  phBuffer                [OUT]           Buffer handle
 *  @return Return MV_FG_SUCCESS on success, and return Error Code on failure.
 *  @remarks The image buffer size is acquired by calling the API MV_FG_GetPayloadSizeEx(). The private information is user-defined.
 ************************************************************************/
MV_FGCTRL_API int __stdcall MV_FG_AnnounceBufferEx(IN STREAMHANDLE hStream, IN void *pBuffer, IN uint64_t nSize, IN void *pPrivate, OUT BUFFERHANDLE *phBuffer);
...
/********************************************************************//**
*  @~chinese
*  @brief  注册图像缓存信息回调函数(SDK外部申请缓存方式)
*  @param  hStream                 [IN]           流通道句柄
*  @param  cbImage                 [IN]           图像缓存信息回调函数
*  @param  pUser                   [IN]           用户自定义数据
*  @return 成功，返回MV_FG_SUCCESS；失败返回错误码
*  @remarks 在回调函数中处理耗时操作会阻塞后续帧信息获取。\n
            注册回调方式和MV_FG_GetImageBuffer互斥。\n
            必须在调用MV_FG_StartAcquisition前注册回调函数。

*  @~english
*  @brief  Register the callback function for image buffer information. This API is valid only when buffers are requested externally by the SDK.
*  @param  hStream                 [IN]            Stream channel handle
*  @param  cbImage                 [IN]            Image buffer information callback function
*  @param  pUser                   [IN]            User-defined data
*  @return Return MV_FG_SUCCESS on success, and return Error Code on failure.
*  @remarks Time-consuming operations in the callback function will block the access to the subsequent frame buffer information. \n
            This API and MV_FG_GetImageBuffer() are mutually exclusive. \n
            You need to call this API to register the callback function before calling API MV_FG_StartAcquisition() .
************************************************************************/
MV_FGCTRL_API int __stdcall MV_FG_RegisterImageCallBack(IN STREAMHANDLE hStream, IN MV_FG_ImageCallBack cbImage, IN void* pUser);
 ...
 *  @return 成功，返回MV_FG_SUCCESS；失败返回错误码
 *  @remark 在Linux平台进行远程显示时，需要OpenGL支持远程直接渲染模式，OpenGL版本高于4.0版本，并且显卡驱动支持高版本OpenGL。
            只有OpenGL模式支持渲染超过4G大小的图像(需要显卡支持)
 ...
 *  @remark When remote display is performed on the Linux platform, OpenGL must support the remote direct rendering mode, the OpenGL version need be later than 4.0, and the graphics card driver needs support a later version of OpenGL.
 ************************************************************************/
MV_FGCTRL_API int __stdcall MV_FG_DisplayOneFrameEx(IN IMAGEHANDLE hImage, IN void* hWnd, IN MV_FG_DISPLAY_FRAME_INFO_EX *pstDisplayFrameInfo, unsigned int enRenderMode);
 ...
 *  @return Return MV_FG_SUCCESS on success, and return Error Code on failure.
 ************************************************************************/
MV_FGCTRL_API int __stdcall MV_FG_SaveTiffToFileEx(IN IMAGEHANDLE hImage, IN OUT MV_FG_SAVE_TIFF_TO_FILE_INFO_EX *pstSaveTiffInfo);
 ...
 *  @return Return MV_FG_SUCCESS on success, and return Error Code on failure.
 ************************************************************************/
MV_FGCTRL_API int __stdcall MV_FG_SavePngToFileEx(IN IMAGEHANDLE hImage, IN OUT MV_FG_SAVE_PNG_TO_FILE_INFO_EX *pstSavePngInfo);
 ...
 *  @~chinese
 *  @brief  图像去紫边
 *  @param  hImage                      [IN]            图像句柄(可以使用相机/流句柄)
 *  @param  pstPurpleFringingParam      [IN]            去紫边参数
 *  @return 成功，返回MV_FG_SUCCESS；失败返回错误码 
 *  @remarks 像素格式仅支持MV_FG_PIXEL_TYPE_RGB8_Packed和PixelType_Gvsp_BGR8_Packed

 *  @~english
 *  @brief  Remove the purple edge from the image.
 *  @param  hImage                      [IN]            Device handle
 *  @param  pstPurpleFringingParam      [IN]            PurpleFringing parameter structure
 *  @return Return MV_FG_SUCCESS on success, and return Error Code on failure.
 *  @remarks Only supports MV_FG_PIXEL_TYPE_RGB8_Packed and PixelType_Gvsp_BGR8_Packed.
 *  ************************************************************************/
MV_FGCTRL_API  int __stdcall MV_FG_PurpleFringing(IN IMAGEHANDLE hImage, IN MV_FG_PURPLE_FRINGING_PARAM* pstPurpleFringingParam);
